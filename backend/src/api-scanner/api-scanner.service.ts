...
export class ApiScannerService {
    ...
    async scanApi(scanRequestDto: ScanRequestDto): Promise<ScanResponseDto> {
        try {
            ...
            await this.createInitialReport(
                scanId,
                scanRequestDto,
                endpoints,
            );
            ...
            this.startScanProcess(scanId, scanRequestDto.baseUrl, endpoints, apiJsonUrl);
        ...
        try {
            ...
            while (retries < maxRetries) {
                try {
                    ...
                    this.logger.warn(`Retry ${retries}/${maxRetries} fetching Swagger document`);
    ...
    private parseSwaggerForEndpoints(swaggerDocument: SwaggerDocument): any[] {
        ...
        try {
            ...
            for (const path in swaggerDocument.paths) {
                for (const method in swaggerDocument.paths[path]) {
                    ...
                    if (typeof endpointInfo !== 'object' || Array.isArray(endpointInfo)) {
                        this.logger.warn(`Invalid endpoint info for ${method.toUpperCase()} ${path}`);
                        continue;
                    }
            ...
            this.logger.error(`Error parsing Swagger for endpoints: ${error.message}`);
        }
    ...
    private async startScanProcess(
        ...
        setTimeout(async () => {
            try {
                ...
                try {
                    this.logger.log(`Starting API scan using OpenAPI definition from ${swaggerJsonUrl}`);
                    const vulnerabilities = await this.zapEngineService.scanApiWithOpenApi(swaggerJsonUrl);
                    await this.updateReportWithResults(scanId, vulnerabilities);
                    return;
                } catch (error) {
                    this.logger.error(`OpenAPI scan failed, falling back to endpoint scanning: ${error.message}`);
                ...
                for (const endpoint of endpoints) {
                    try {
                        ...
                        this.logger.log(`Scanning endpoint ${targetUrl} with method ${endpoint.method}`);
                        ...
                        this.logger.error(`Error scanning endpoint ${endpoint.path}: ${error.message}`);
    ...
    private async updateReportWithResults(
        ...
        this.logger.log(`Scan ${scanId} completed with ${vulnerabilities.length} vulnerabilities found`);
    }
